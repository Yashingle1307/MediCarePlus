    ...
    async function fetchDoctor() {
      try {
        ...
        if (Array.isArray(json?.data)) {
          raw = json.data.find((doc) => doc._id === doctorId);
        } else {
          raw = json?.data || json?.doctor || json;
        }

        if (!raw) throw new Error("Doctor not found in response");

        // ⭐ THIS LINE FIXES EVERYTHING
        let d = raw._doc ? raw._doc : raw;





        if (!d) throw new Error("Doctor not found in response");
        ...
        d = {
          ...
          about: d.about ?? "",
          fee: d.fee ?? d.fees ?? 0,
          patients: d.patients ?? d.totalPatients ?? 0,
          success: d.success ?? 0,
          rating: d.rating ?? 0,
