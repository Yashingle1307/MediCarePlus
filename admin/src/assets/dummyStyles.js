...
export const serviceAppointmentsStyles = {
  container: "p-4 sm:p-6 md:p-6 font-serif max-w-7xl mx-auto",

  headerContainer:
    "flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6 mb-8 w-full",

  headerTitleContainer: "min-w-0 flex flex-col",

  headerTitle:
    "text-2xl md:text-3xl font-extrabold text-emerald-800 tracking-tight",
  ...
  searchContainer:
    "w-full lg:w-auto flex flex-col lg:items-end gap-2",

  searchInputWrapper:
    "flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full lg:w-auto",

  searchLabel:
    "relative block w-full sm:w-72",
  ...
  clearSearchButton:
    "absolute right-3 top-1/2 -translate-y-1/2 rounded-full p-1 hover:bg-gray-100",

  clearSearchIcon:
    "w-4 h-4 text-gray-500",

  statusFilterSelect:
    "text-sm px-3 py-2 cursor-pointer rounded-full border border-emerald-400 bg-white",

  /* ⭐ RESULT TEXT ALIGNMENT FIX */
  searchInfo:
    "mt-2 text-xs text-gray-500 flex items-center justify-between lg:justify-end gap-6 w-full",

  refreshButton:
    "text-xs text-emerald-600 hover:underline whitespace-nowrap",
  ...
  /* ⭐ GRID ALIGNMENT WITH NAVBAR WIDTH */
  gridContainer:
    "grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 items-stretch mt-6",
  ...
  detailsContainer:
    "mt-4 flex flex-col gap-3 text-gray-700",
