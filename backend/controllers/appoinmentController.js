    ...
    try {
        ...
        if (!resolvedCreatedBy) {
            return res.status(401).json({
                success: false,
                message: "createdBy is required either in query or from auth"
            });
        }
    ...
    try {
        ...
        if (!doctorId || !patientName || !mobile || !date || !time) {
            return res.status(400).json({
                success: false,
                message: "Missing required fields: doctorId, patientName, mobile, date, time"
            });
        }
        ...
        try {
            ...
            console.error("Doctor LookUp failded", e?.message || e);
        }
        ...
        const successUrl = `${frontBase}/appointments/success?session_id={CHECKOUT_SESSION_ID}`;
        const cancelUrl = `${frontBase}/appointments/cancel`;
...
export const updateAppointment = async (req, res) => {
    try {
        ...
        const updated = await Appointment.findByIdAndUpdate(id, update, { new: true, runValidators: true }).populate
            ({ path: "doctorId", select: "name imageUrl" }).lean();
...
export const getStats = async (req, res) => {
    try {
        ...
        const recent = await Appointment.find({ createdAt: { $gte: sevenDaysAgo } });
...
export const getAppointmentByDoctor = async (req, res) => {
    try {
        ...
        if (!doctorId) {
            return res.status(400).json({
                success: false,
                message: "doctorId is required in params"
            });
        }
        ...
        const items = await Appointment.find(filter)
            ...
            .populate({
                path: "doctorId",
                select: "name specialization imageUrl image"
            })
            .lean();
...
export async function getRegisteredPatientsCount(req, res) {
    try {
        const count = await clerkClient.users.count();
        return res.json({ success: true, count });
    } catch (err) {
        console.error("getRegisteredPatientsCount error:", err);
