...
export const createServiceAppointment = async (req, res) => {
    try {
        ...
        const successUrl = `${frontendBase}/service-appointments/success?session_id={CHECKOUT_SESSION_ID}`;
        const cancelUrl = `${frontendBase}/service-appointments/cancel`;
...
export const confirmServicePayment = async (req, res) => {
  try {
    ...
    // 🔁 fallback if appointmentId exists in metadata
    ...
    console.error("Error in confirmServicePayment:", err);
    return res.status(500).json({
      success: false,
...
export const getServiceAppointments = async (req, res) => {
    try {
        ...
        console.error("Error in getServiceAppointments:", err);
        return res.status(500).json({ success: false, message: "Internal server error" });
    }
};

// to service appointment by id
export const getServiceAppointmentById = async (req, res) => {
    try {
        ...
        console.error("Error in getServiceAppointmentById:", err);
        return res.status(500).json({ success: false, message: "Internal server error" });
    }
};
...
export const updateServiceAppointment = async (req, res) => {
    try {
        ...
        console.error("Error in updateServiceAppointment:", err);
        return res.status(500).json({ success: false, message: "Internal server error" });
    }
};
...
export const cancelServiceAppointment = async (req, res) => {
    try {
        ...
        console.error("Error in cancelServiceAppointment:", err);
        return res.status(500).json({ success: false, message: "Internal server error" });
    }
};

// to get statistics of service appointments
export const getServiceAppointmentStats = async (req, res) => {
    try {
        ...
        console.error("Error in getServiceAppointmentStats:", err);
        return res.status(500).json({ success: false, message: "Internal server error" });
    }
};
...
export const getServiceAppointmentsByPatient = async (req, res) => {
    try {
        ...
        if (!resolvedCreatedBy && !mobile) {
            return res.json({
                success: false,
                message: "Authentication required to get patient appointments"
            });
        }
